...
class GestureToJoy(Node):
    def __init__(self):
        ...
        # Subscriber que recebe o nome/estado do gesto vindo do Unity (ou outro sistema)
        self.gesture_subscriber = self.create_subscription(
            String,
            'hand_gesture',    # Tópico publicado pela Unity
            self.gesture_callback,
            10
        )

        # Publisher que envia comandos do tipo Joy
        self.joy_publisher = self.create_publisher(Joy, 'joy', 10)

        # Inicializa a estrutura do Joy (e.g., 2 eixos e 0 botões)
        self.joy_msg = Joy()
    ...
    def gesture_callback(self, gesture_msg):
        ...
        # Mapeie o gesto para valores de eixos do Joy (exemplo ilustrativo)
        if gesture == "OPEN_HAND":
            self.joy_msg.axes = [0.0, 0.0]
        elif gesture == "POINTING":
            self.joy_msg.axes = [1.0, 0.0]
        elif gesture == "PINCH":
            self.joy_msg.axes = [0.0, 1.0]
        elif gesture == "FIST":
